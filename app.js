...
let isAuthenticated = false; // Global flag for user's login status

async function syncToServer(type, key, value) {
    if (!isAuthenticated) return; // Don't sync if not logged in
    ...
    try {
        ...
        // Optional: show a non-intrusive notification to the user
...
async function checkAuthStatus() {
    // --- Get all login/logout related elements ---
    ...
    const allLoginButtons = [loginButton, mobileLoginButton];
    ...
    const showLoggedInState = (user) => {
        ...
        mobileUserDisplayName.textContent = user.displayName || user.email;
    ...
    try {
        // 1. Check which providers are configured on the backend
        ...
        // 2. Populate login buttons dynamically
        loginProviderButtons.innerHTML = '';
        if (providers.length > 0) {
            providers.forEach(provider => {
                ...
                loginProviderButtons.appendChild(a);
            });
        } else {
            loginProviderButtons.textContent = 'No login providers are configured on the server.';
        }


        // 3. Check the user's authentication status
        ...
        if (data.user) {
            ...
            await syncFromServer(); // Sync data from server on login
            ...
            isAuthenticated = false;
            showLoggedOutState();
            setupLoginModal(allLoginButtons, loginModal, closeLoginModalButton);
        }
    } catch (error) {
        console.error('Error during auth check:', error);
        showTopNotification(error.message, 'error', 5000); // Show error for longer
        isAuthenticated = false;
        showLoggedOutState(); // Show logged-out state
        // Disable login buttons and add a tooltip
...
async function syncFromServer() {
    try {
        ...
        if (serverData.configs) {
            ...
            // Simple merge: server data overwrites local
        ...
        if (serverData.cardStats) {
            for (const key in serverData.cardStats) {
                ...
                if (localStat) {
                    // Use the more robust merging logic from import/export
        ...
        // Reload the page to apply the synced configuration and data
...
function initializeApp() {
    ...
    async function saveSkill() {
        ...
        if (editingSkillId) {
            ...
            currentConfig.activeSkills.push(skill.id);
        }
    ...
    async function createPresetSkills() {
        ...
        try {
            ...
            for (const p of presets) {
                ...
                const created = await createSkill(skillData);
                currentConfig.skills.push(created);
                // Also enable the new preset skill by default
                if (!currentConfig.activeSkills.includes(created.id)) {
                    currentConfig.activeSkills.push(created.id);
                }
            }
    ...
    async function saveCurrentConfig() {
        ...
        await syncToServer('configs', 'flashcard-configs', configs);
    ...
    function populateSkillSelector(container, isMobile = false) {
        ...
        const currentConfigName = configSelector.value;
        const currentConfig = configs[currentConfigName];
        if (!currentConfig || !currentConfig.skills || currentConfig.skills.length === 0) return;

        const roleToColumnMap = currentConfig.roleToColumnMap || {};

        // --- Add "Toggle All" checkbox ---
        const toggleAllId = `toggle-all-skills-${isMobile ? 'mobile' : 'desktop'}`;
        const toggleAllDiv = document.createElement('div');
        toggleAllDiv.className = 'toggle-all-skill-container';
        toggleAllDiv.innerHTML = `<input type="checkbox" id="${toggleAllId}"><label for="${toggleAllId}">Toggle All</label>`;
        container.appendChild(toggleAllDiv);

        const allCheckboxes = [];

        // --- Populate individual skill checkboxes ---
        currentConfig.skills.forEach((skill, index) => {
            ...
            // --- Disable skill if required roles are not assigned ---
            const requiredRoles = new Set([...skill.front, ...skill.back]);
            if (skill.validationColumn !== 'none') requiredRoles.add(skill.validationColumn);
            if (skill.ttsFrontColumn !== 'none') requiredRoles.add(skill.ttsFrontColumn);
            if (skill.ttsBackColumn !== 'none') requiredRoles.add(skill.ttsBackColumn);

            const missingRoles = [...requiredRoles].filter(role => !roleToColumnMap[role] || roleToColumnMap[role].length === 0);

            if (missingRoles.length > 0) {
                input.disabled = true;
                label.title = `Disabled: Requires unassigned role(s): ${missingRoles.join(', ')}`;
        ...
        // --- Set checked status from config ---
        if (currentConfig.activeSkills) {
        ...
        // --- Add event listener for "Toggle All" ---
        const toggleAllCheckbox = document.getElementById(toggleAllId);
        if (toggleAllCheckbox) {
            // Set initial state of "Toggle All"
            const allEnabledAndChecked = allCheckboxes.every(cb => cb.disabled || cb.checked);
            toggleAllCheckbox.checked = allEnabledAndChecked;
            ...
            toggleAllCheckbox.addEventListener('change', (e) => {
                ...
                // Trigger the main handler to save and update the view

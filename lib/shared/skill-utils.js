...
function buildCanonicalString(skill) {
    ...
    // Add skill-affecting transforms to the canonical string
    if (skill.transforms) {
        const transformParts = [];
        const sides = Object.keys(skill.transforms).sort();
        for (const side of sides) {
            const roles = Object.keys(skill.transforms[side]).sort();
            for (const role of roles) {
                const transform = skill.transforms[side][role];
                const affectingTransforms = [];
                if (transform.hideString) {
                    affectingTransforms.push(`hideString:${transform.hideStringColumn || 'none'}`);
                }
                if (transform.suppressParentheses) {
                    affectingTransforms.push('suppressParentheses:true');
                }

                if (affectingTransforms.length > 0) {
                    // Sort the affecting transforms for determinism
                    affectingTransforms.sort();
                    transformParts.push(`${side}|${role}:${affectingTransforms.join(',')}`);
                }
            }
        }
        if (transformParts.length > 0) {
            parts.push(`transforms:${transformParts.join('&')}`);
        }
    }
...
export async function createSkill(skillData) {
    ...
    skill.transforms = skillData.transforms || { front: {}, back: {} };
    return skill;
}

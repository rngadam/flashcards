...
export function updateState(newState) {
    const oldAuth = state.isAuthenticated;
    Object.assign(state, newState);
    if (oldAuth !== state.isAuthenticated) {
        Bus.publish('auth:state:change', { isAuthenticated: state.isAuthenticated });
    }
}
...
/**
 * Pops the last state from the view history stack without mutating the state.
 * @returns {{lastState: {cardIndex: number, skillId: string} | undefined, newHistory: Array}}
 */
export function popFromViewHistory() {
    if (state.viewHistory.length === 0) {
        return { lastState: undefined, newHistory: [] };
    }
    const newHistory = state.viewHistory.slice(0, -1);
    const lastState = state.viewHistory[state.viewHistory.length - 1];

...
export async function checkWritingAnswer() {
    try { logger.log('ui', 'checkWritingAnswer.start'); } catch (e) { /* ignore */ }
    ...
    if (userAnswer === '') {
        try { logger.log('ui', 'checkWritingAnswer.emptyAnswer'); } catch (e) { /* ignore */ }
        await markCardAsKnown(false);
        if (!dom.card.classList.contains('flipped')) {
            try { logger.log('ui', 'checkWritingAnswer.empty.flipping'); } catch (e) { /* ignore */ }
            flipCard();
        }
        dom.writingInput.disabled = true;
        setTimeout(() => showNextCard({ forceNew: true }), delay);
        return;
    }

    try { logger.log('ui', 'checkWritingAnswer.nonEmptyAnswer', { userAnswer }); } catch (e) { /* ignore */ }
    ...
    setTimeout(() => showNextCard({ forceNew: !isCorrect }), delay);
}
...
export function generateMultipleChoiceOptions() {
    ...
    try { logger.log('ui', 'mc.generate', { options, correctAnswer }); } catch (e) { /* ignore */ }
    options.forEach((option, index) => {
        ...
        button.addEventListener('click', () => {
            try { logger.log('ui', 'mc.click', { selected: option }); } catch (e) { /* ignore */ }
...
export function checkMultipleChoiceAnswer(selectedAnswer, correctAnswer) {
    try { logger.log('ui', 'checkMultipleChoiceAnswer.start'); } catch (e) { /* ignore */ }
    ...
    try { logger.log('ui', 'mc.answer', { selectedAnswer, correctAnswer }); } catch (e) { /* ignore */ }
    ...
    (async () => {
        try {
            ...
            try { logger.log('error', 'mc.markCardError', { error: e && e.message }); } catch (_) { /* ignore */ }
        }

        // Flip card to reveal answer if needed (may be async)
        try {
            if (!dom.card.classList.contains('flipped')) {
                try { logger.log('ui', 'mc.flipping'); } catch (e) { /* ignore */ }
                await flipCard();
            }
            try { logger.log('ui', 'mc.postFlip', { cardFlipped: dom.card.classList.contains('flipped') }); } catch (e) { /* ignore */ }
        } catch (e) {
            try { logger.log('error', 'mc.flipError', { error: e && e.message }); } catch (_) { /* ignore */ }
        }
        ...
        try {
            ...
            setTimeout(() => {
                try {
                    logger.log('ui', 'mc.advancing');
                    showNextCard({ forceNew: !isCorrect });
                } catch (e) { try { logger.log('error', 'mc.nextCardError', { error: e && e.message }); } catch (_) { /* ignore */ } }
            }, delay);
        } catch (e) {
            try { logger.log('error', 'mc.nextCardScheduleError', { error: e && e.message }); } catch (_) { /* ignore */ }
        }
    })();
}
...
async function checkVoiceAnswer(transcript) {
    try { logger.log('ui', 'checkVoiceAnswer.start', { transcript }); } catch (e) { /* ignore */ }
    const skillConfig = getCurrentSkillConfig();
    const validationRole = skillConfig.validationColumn;
    const roleToColumnMap = (state.configs[dom.configSelector.value] || {}).roleToColumnMap || {};
    const validationColumnIndices = roleToColumnMap[validationRole] || [];
    ...
    try { logger.log('ui', 'checkVoiceAnswer.result', { isCorrect }); } catch (e) { /* ignore */ }
    ...
    // Always stop recognition and advance, regardless of correctness
    stopVoiceRecognition();

    if (!dom.card.classList.contains('flipped')) {
        try { logger.log('ui', 'checkVoiceAnswer.flipping'); } catch (e) { /* ignore */ }
        flipCard();
    }
    ...
    const delay = parseInt(currentConfig.voiceCorrectDelay, 10) || 1000;
    setTimeout(() => {
        try { logger.log('ui', 'checkVoiceAnswer.advancing'); } catch (e) { /* ignore */ }
        showNextCard({ forceNew: !isCorrect });
    }, delay);
}
...
import { mergeCardStats } from './stats-utils.js';
...
let showNextCard;
let showConflictResolutionModal;
...
export function initAuth(dependencies) {
    ...
    showConflictResolutionModal = dependencies.showConflictResolutionModal;
}
...
export async function syncToServer(data) {
    ...
    try {
        ...
        if (result.conflicts && result.conflicts.length > 0) {
            showTopNotification(`${result.conflicts.length} sync conflicts detected.`, 'warning');

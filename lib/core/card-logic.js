...
import { getLenientString, transformSlashText, stripParentheses } from '../string-utils.js';
...
export function initCardLogic(dependencies) {
    ...
    saveCardStats = dependencies.saveCardStats;
    ...
    handleIDontKnow = dependencies.handleIDontKnow;
    generateMultipleChoiceOptions = dependencies.generateMultipleChoiceOptions;
    checkMultipleChoiceAnswer = dependencies.checkMultipleChoiceAnswer;
...
export async function getTextForRoles(roles, baseLanguageIndex = -1) {
    const currentConfig = state.configs[dom.configSelector.value];
        ...
        for (const colIndex of columnIndices) {
            ...
            if (roleKey === 'RELATED_WORD') {
                ...
                const studiedWords = relatedWords.filter((word, i) => {
                    ...
                    return Object.values(stats.skills).reduce((sum, s) => sum + (s.viewCount || 0), 0) > 0;
                });
                if (studiedWords.length > 0) {
                    textParts.push({ text: transformSlashText(studiedWords.join(', ')), role: roleKey });
                }
            } else {
...
export async function displayCard(index, { isNavigatingBack = false, reason = {} } = {}) {
    ...
    const newFrontParts = await getTextForRoles(frontRoles, state.currentRandomBaseIndex);
    const newBackParts = await getTextForRoles(backRoles, state.currentRandomBaseIndex);
    const newTtsFrontParts = await getTextForRoles(ttsFrontRole, state.currentRandomBaseIndex);
    const newTtsBackParts = await getTextForRoles(ttsBackRole, state.currentRandomBaseIndex);
...
export async function showPrevCard() {
    const prevState = popFromViewHistory();
    if (prevState) {
        updateState({ currentSkillId: prevState.skillId });

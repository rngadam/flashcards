...
import { sanitizeCardStats, createDefaultSkillStats } from '../shared/validation.js';
...
function renderCardFaces(frontParts, backParts, skillConfig) {
    ...
    dom.cardFrontContent.style.fontSize = '';
...
export async function flipCard() {
    try { logger.log('ui', 'flipCard.start'); } catch (e) { /* ignore */ }
    if (!dom.card) {
        try { logger.log('ui', 'flipCard.noDomCard'); } catch (e) { /* ignore */ }
        return;
    }

    if (!dom.multipleChoiceContainer.classList.contains('hidden') && !dom.multipleChoiceContainer.classList.contains('answered')) {
        try { logger.log('ui', 'flipCard.mcNotAnswered'); } catch (e) { /* ignore */ }
    ...
    const skillConfig = getCurrentSkillConfig();
    if (!skillConfig) {
        try { logger.log('ui', 'flipCard.noSkillConfig'); } catch (e) { /* ignore */ }
        return;
    }

    const isFlippingToFront = dom.card.classList.contains('flipped');
    try { logger.log('ui', 'flipCard.beforeToggle', { classList: Array.from(dom.card.classList), isFlippingToFront }); } catch (e) { /* ignore */ }
    dom.card.classList.toggle('flipped');
    try { logger.log('ui', 'flipCard.afterToggle', { classList: Array.from(dom.card.classList) }); } catch (e) { /* ignore */ }
...
export async function saveCardStats(cardKey, stats) {
    ...
    // Persist locally first for immediate consistency
    await set(cardKey, stats);
    // Dispatch to DAL for adapters to handle syncing (IndexedDB adapter will
    // also persist, but the DAL allows HTTP adapter to sync to server).
    try {
        await DAL.dispatch('data:card:stats:save', { cardStats: { [cardKey]: stats } });
    } catch (e) {
        // If sync fails, publish a failure via existing syncToServer fallback
        if (typeof syncToServer === 'function') {
            try {
                await syncToServer({ cardStats: { [cardKey]: stats } });
            } catch (_) {
                // swallow - higher layers handle unresolved conflicts
            }
        }
    }
}
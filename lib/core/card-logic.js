...
export async function markCardAsKnown(known) {
    try {
        ...
        if (!cardKey) {
            console.error("markCardAsKnown: Could not determine cardKey.");
            return;
        }
        ...
        console.error("Error in markCardAsKnown:", error);
        // Log to our logger as well, but don't let it crash the process
        try {
            logger.log('error', 'markCardAsKnown.error', { message: error.message, stack: error.stack });

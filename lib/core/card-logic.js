        ...
        for (const colIndex of columnIndices) {
            ...
            if (transform) {
                ...
                if (transform.hideString && transform.hideStringColumn && transform.hideStringColumn !== 'none') {
                    const stringToHide = getRawTextForRole(transform.hideStringColumn);
                    if (stringToHide) {
                        const trimmedStringToHide = stringToHide.trim();
                        if (trimmedStringToHide) {
                            const regex = new RegExp(trimmedStringToHide.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g');
...
function renderCardFaces(frontParts, backParts, skillConfig) {
    ...
    if (isAudioOnly(skillConfig)) {
        ...
        frontParts.forEach(part => {
            ...
            if (transform?.font) {
                partDiv.style.fontFamily = transform.font === 'Random' ? 'inherit' : transform.font; // Logic for random can be added
            }
    ...
    backParts.forEach(part => {
        ...
        if (transform?.font) {
            partDiv.style.fontFamily = transform.font === 'Random' ? 'inherit' : transform.font;
        }
...
export async function flipCard({ forceRerender = false } = {}) {
    if (!dom.card) return;
    ...
    const shouldRender = isFlippingToFront || forceRerender;
